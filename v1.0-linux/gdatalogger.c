...
int gDataLogger_DeclareVariable(PGDATALOGGER pgDataLogger, char *varname, char *varunit, int queuesize)
{
	...
	pgDataLogger->Variables[pgDataLogger->m_NumberOfVariables].CircularQueue = (double *) malloc(queuesize * sizeof(double));
...
int gDataLogger_InsertVariable(PGDATALOGGER pgDataLogger, char *varname, double* varvalue, int size)
{
	...
	for(i=0;i<size;++i){
		...
		pgDataLogger->Variables[varindex].CircularQueue[queueindex] = varvalue[i];
		if(!FlagQueueFull){
